...
function deletePlaylist(row) {
  ElMessageBox.confirm('确认移除' + row.playlist_Name + '？', {
...
<template>
  <div>
    <div class="songList_mod" v-if="LikePlaylistExist">
      <div class="listTitle_mod">
        <span class="songTitle">名称</span>
      ...
      <div class="listSong_mod" v-for="(item,index) in likeList ">
        ...
        <div class="songName">
          ...
          <span @click="toPlaylist(item.playlist_ID)">{{ item.playlist_Name }}</span>
        </div>
        <div class="create_By">
          <span>{{ item.create_By }}</span>
        </div>
        <div class="plCreate_By">
          <span>{{ item.create_Time }}</span>
        </div>
        <div class="option_mod">
          <div class="deletePlaylist">
            <div class="delete_button" @click="deletePlaylist(item)">
    ...
    <div class="likePlaylistNull_mod" v-if="!LikePlaylistExist">
...
/*鼠标悬浮显示操作按钮*/
.listSong_mod:hover .option_mod .deletePlaylist {
  display: flex;
...
.likePlaylistNull_mod svg {
  width: 300px;
  height: 300px;
}

.likePlaylistNull_mod span {
  font-family: STXihei, serif;
  font-size: 15px;
  font-weight: 900;
}
