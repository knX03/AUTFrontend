...
import {aDeleteLikeSong, aLikeSong, aSelectLikeSong, aSelectSongByPlaylist} from "@/api/api.js";
...
let playlistSongExist = ref(false)
...
//todo songDetail页面未完成
const toSong = (song_ID) => {
  router.push({
    path: '/songDetail',
    query: {song_ID}
  })
...
<template>
  <div class="songList_mod" v-if="playlistSongExist">
  ...
  <div class="playlistNull_mod" v-if="!playlistSongExist">
...
.ListBody_mod:hover .playButton_mod {
  display: flex;
}

.ListBody_mod:hover .SN_mod {
  display: none;
}
...
.playlistNull_mod svg {
  width: 300px;
  height: 300px;
}

.playlistNull_mod span {
  font-family: STXihei, serif;
  font-size: 15px;
  font-weight: 900;
}
